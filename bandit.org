...
let set the shell to /bin/bash so we can execute normal commands (from within 
more): ':set shell=/bin/bash'. In the home folder you can now find a file called
...
case '/bin/bash'. From here simply 'cat' out the password of bandit33.
